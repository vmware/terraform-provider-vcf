...
func TestValidateNsxEdgePassword(t *testing.T) {
	t.Run("Validate NSX Edge password", func(t *testing.T) {
		var passwordTests = []struct {
			password    string
			expectedErr string
		}{
			{"Test1!", "the password must be at least 12 characters long"},
			{"testpassword1!", "the password must contain at least one upper case letter"},
			{"TESTPASSWORD1!", "the password must contain at least one lower case letter"},
			{"Testpassword!", "the password must contain at least one digit"},
			{"Testpassword1", "the password must contain at least one special symbol"},
		...
		for _, passTest := range passwordTests {
			_, err := ValidateNsxEdgePassword(passTest.password, "")
			if len(err) == 0 {
				t.Errorf("failed. expected one error for password %s, but got zero", passTest.password)
				break
			}
...
func TestValidateCidrIPv4AddressSchema(t *testing.T) {
	t.Run("Validate CIDR IPv4 address schema", func(t *testing.T) {
		var ipTests = []struct {
			ip          string
			expectError bool
		}{
		...
		for _, ipTest := range ipTests {
			_, err := ValidateCidrIPv4AddressSchema(ipTest.ip, "")
